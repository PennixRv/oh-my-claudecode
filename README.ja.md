...
> **注意: パッケージ名について** — プロジェクトのブランド名は **oh-my-claudecode**（リポジトリ、プラグイン、コマンド）ですが、npmパッケージは [`oh-my-claude-pennix`](https://www.npmjs.com/package/oh-my-claude-pennix) として公開されています。npm/bunでCLIツールをインストールする場合は `npm install -g oh-my-claude-pennix` を使用してください。
