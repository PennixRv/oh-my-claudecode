...
async function cleanupOldReports(cwd) {
  ...
  try {
    ...
    for (const file of files) {
      const filePath = join23(reportsDir, file);
...
async function rollbackUnpersistedNativeWorktreeStartup(teamName, cwd, cause) {
  ...
  try {
    ...
    await writeFile8(join24(teamRoot, "startup-failure.json"), JSON.stringify({
...
async function startTeamV2(config) {
  ...
  await mkdir11(join24(getOmcRoot(leaderCwd), "state", "team", sanitized, "mailbox"), { recursive: true });
...
`
  ...
  for (let i = 0; i < config.tasks.length; i++) {
    ...
    await mkdir11(join24(taskFilePath, ".."), { recursive: true });
...
import { existsSync as existsSync24, mkdirSync as mkdirSync4, readFileSync as readFileSync12, writeFileSync as writeFileSync4 } from "fs";
...
import { dirname as dirname17, join as join25, resolve as resolvePath } from "node:path";
      ...
      case "transition-task-status": {
        ...
        if (to === "completed" || to === "failed") {
          ...
          try {
            ...
            const taskPath2 = join28(cwd, ".omc", "state", "team", teamName, "tasks", `task-${taskId}.json`);
...
init_worktree_paths();
import { readdirSync as readdirSync6, readFileSync as readFileSync11, existsSync as existsSync23 } from "fs";
